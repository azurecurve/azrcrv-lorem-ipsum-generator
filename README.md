# [Lorem Ipsum Generator](https://development.azurecurve.co.uk/classicpress-plugins/lorem-ipsum-generator/)
![Plugin Banner](/assets/pluginimages/banner-1544x500.png)

# Description

Allows users to generate paragraphs of "lorem ipsum" text.

Options are maintained via a Settings page on the **azurecurve** menu.

Lorem Ipsum forms are placed using the `lorem-ipsum-generator` shortcode and can have a number of parameters supplied to override the defaults from the options page; each shortcode must have an `id` parameter supplied. Available parameters are:
 * `start-with-lorem-ipsum` - determines if first paragraph begins with "lorem ipsum dolor sit amet".
 * `maximum-paragraph-count` - maximum number of paragraphs to be calculated; user can't increment above tis level.
 * `paragraph-count` - default number of paragraphs to be calculated.
 * `maximum-word-count` - maximum number of words to be generated; user can't increment above tis level.
 * `word-count` - default number of words to be generated.
 * `text-before` - text to display before form.
 * `text-after` - text to display after form.
 * `label-word-count` - label for password length field.
 * `label-paragraph-count` - label for number of passwords to generate field.
 

Example shortcode usage:
```
[lorem-ipsum-generator id="lorem-ipsum-1"  text-before="The lore ipsum generator below can be used to produce paragraphs of Lorem Ipsum text." paragraph-count=3 word-count=200]
```
 ...
 * Download the plugin from [GitHub](https://github.com/azurecurve/azrcrv-lorem-ipsum-generator/releases/latest/).
